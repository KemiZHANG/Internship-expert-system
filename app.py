...
def build_export_markdown(result, fact_by_id, recommendations, translations, lang, career_result=None):
    readiness = result.get("readiness", {})
    lines = [
        "# Internship Readiness Expert System Result",
        "",
        "## Overall Readiness",
        f"- Level: {readiness.get('level', result.get('score_based_level', '-'))}",
        f"- Score: {readiness.get('score', result.get('readiness_score', 0))} / 100",
        f"- Main priority area: {readiness.get('main_priority_area', result.get('main_priority_area', '-'))}",
        f"- Main advice: {readiness.get('main_advice', '-')}",
        "",
        "## Selected Facts",
    ]
    ...
    for fact_id in result.get("selected_facts", []):
        ...
        lines.append(f"- {fact_id}: {localized_fact_text(fact, lang)}")

    lines.extend(["", "## Triggered Rules"])
    for rule in result.get("triggered_rules", []):
        lines.append(
            f"- {rule['id']} ({rule['type']}): {rule['name']} -> "
            f"{rule['conclusion']} | {readable_conditions(rule.get('conditions', {}))}"
        )

    lines.extend(["", "## Intermediate Decisions"])
    for decision_id in result.get("inferred_decisions", []):
        lines.append(f"- {decision_id}")

    readiness_decisions, advice_decisions = split_final_decisions(result)
    lines.extend(["", "## Final Readiness Decision"])
    for decision in readiness_decisions:
        text = t(translations, lang, f"decision.{decision['id']}", decision["text"])
        lines.append(f"- {decision['id']}: {text} ({decision.get('source', 'rule_based')})")

    if result.get("score_fallback_decisions", []):
        lines.append("")
        lines.append(
            "Note: This readiness level was generated by score-based fallback because "
            "no rule-based readiness decision was triggered."
        )

    lines.extend(["", "## Specific Advice Decisions"])
    for decision in advice_decisions:
        text = t(translations, lang, f"decision.{decision['id']}", decision["text"])
        lines.append(f"- {decision['id']}: {text} (Rule ID: {decision.get('rule_id', 'N/A')})")

    if career_result:
        major_name = localized_dict_text(
            career_result.get("major", {}),
            lang,
            "display_text",
            career_result.get("major_id", ""),
        )
        lines.extend(["", "## Career Path Recommendation", f"- Selected major: {major_name}"])
        for track in career_result.get("recommended_tracks", []):
    ...
    for decision in result.get("final_decisions", []):
        rec = recommendations.get(decision["id"])
        if not rec or decision["id"] in shown:
            continue
        shown.add(decision["id"])
        lines.append(f"### {decision['id']} - {localized_recommendation(rec, lang, 'title')}")
        lines.append(f"- Summary: {localized_recommendation(rec, lang, 'summary')}")
        lines.append(f"- Recommendation: {localized_recommendation(rec, lang, 'recommendation')}")
        lines.append("- Next steps:")
        for step in localized_recommendation(rec, lang, "next_steps") or []:
            lines.append(f"  - {step}")

    lines.extend(["", "## Explanation Chain"])
    explanation_chain = result.get("explanation_chain", {"steps": []})
    lines.append(explanation_chain.get("start", ""))
    for step_number, step in enumerate(explanation_chain.get("steps", []), start=1):
        lines.append(
            f"{step_number}. {step['rule_id']} - {step['rule_name']}: "
            f"{step['conclusion']} ({readable_conditions(step['conditions'])})"
        )
    lines.append(explanation_chain.get("end", ""))
    return "\n".join(lines)


def render_export_panel(result, fact_by_id, recommendations, translations, lang, key_suffix, career_result=None):
    export_text = build_export_markdown(result, fact_by_id, recommendations, translations, lang, career_result)
    st.markdown(
        f"""
        <div class="export-panel">
            <div class="summary-value">{escape(t(translations, lang, "export.title"))}</div>
            <p class="soft-note">{escape(t(translations, lang, "export.help"))}</p>
        </div>
        """,
        unsafe_allow_html=True,
    )
    st.download_button(
        t(translations, lang, "export.download"),
        data=export_text,
        file_name="internship_readiness_explanation.md",
        mime="text/markdown",
        key=f"download_explanation_{key_suffix}",
    )


def render_why_not_high(result, translations, lang):
    working_memory = set(result.get("working_memory", []))
    readiness = result.get("readiness", {})
    is_high = readiness.get("level") == "High Internship Readiness"
    criteria = [
        ("D1", t(translations, lang, "why.criteria_materials")),
        ("D16", t(translations, lang, "why.criteria_portfolio")),
        ("D3", t(translations, lang, "why.criteria_technical")),
        ("D5", t(translations, lang, "why.criteria_interview")),
        ("D7", t(translations, lang, "why.criteria_direction")),
        ("D9/D10", t(translations, lang, "why.criteria_action")),
    ]
    blockers = [
        ("D4", t(translations, lang, "why.blocker_technical")),
        ("D6", t(translations, lang, "why.blocker_interview")),
        ("D8", t(translations, lang, "why.blocker_direction")),
    ]

    st.subheader(t(translations, lang, "explain.why_not_high"))
    st.caption(t(translations, lang, "explain.why_not_high_help"))
    if is_high:
        st.success(t(translations, lang, "why.high_satisfied"))

    cards = []
    for decision_id, label in criteria:
        if decision_id == "D9/D10":
            met = "D9" in working_memory or "D10" in working_memory
            detail = "D9 or D10"
        else:
            met = decision_id in working_memory
            detail = decision_id
        status_key = "why.status_met" if met else "why.status_missing"
        css_class = "pass" if met else "missing"
        cards.append(
            f"""
            <div class="why-card {css_class}">
                <div class="why-status">{escape(t(translations, lang, status_key))}</div>
                <div class="why-title">{escape(label)}</div>
                <div class="why-detail">{escape(detail)}</div>
            </div>
            """
        )

    for decision_id, label in blockers:
        if decision_id in working_memory:
            cards.append(
                f"""
                <div class="why-card missing">
                    <div class="why-status">{escape(t(translations, lang, "why.status_blocker"))}</div>
                    <div class="why-title">{escape(label)}</div>
                    <div class="why-detail">{escape(decision_id)}</div>
                </div>
                """
            )

    st.markdown(f'<div class="why-grid">{"".join(cards)}</div>', unsafe_allow_html=True)


def render_rule_diagram(translations, lang):
    rows = [
        ("F1 + F2 + F12 + (F22 OR F23)", "AND/OR", "D1", t(translations, lang, "diagram.materials")),
        ("F3 + F4 + F25 + F26 + (F24 OR F27 OR F28)", "AND/OR", "D16", t(translations, lang, "diagram.portfolio")),
        ("F5 + F6 + F29 + F30 + F31 + F32 + NOT F15", "AND/NOT", "D3", t(translations, lang, "diagram.technical")),
        ("F33 + F34 + F35 + F36 + F8 + D18", "AND", "D5", t(translations, lang, "diagram.interview")),
        ("F9 + F14 + F37 + F38", "AND", "D7", t(translations, lang, "diagram.direction")),
        ("D1 + D16 + D3 + D5 + D7 + (D9 OR D10) + NOT D4/D6/D8", "AND/OR/NOT", "FD1", t(translations, lang, "diagram.high")),
        ("D2 + D4 OR D17 + D4 + D8", "AND/OR", "FD3", t(translations, lang, "diagram.low")),
    ]
    html_rows = []
    for facts, operator, decision_id, decision_label in rows:
        html_rows.append(
            f"""
            <div class="logic-row">
                <div class="logic-node fact">{escape(facts)}</div>
                <div class="logic-arrow">-></div>
                <div class="logic-node operator">{escape(operator)}</div>
                <div class="logic-arrow">-></div>
                <div class="logic-node decision">{escape(decision_id)} - {escape(decision_label)}</div>
            </div>
            """
        )
    st.markdown(
        ...
        <div class="logic-board">
            <div class="summary-value">{escape(t(translations, lang, "kb.rule_diagram_title"))}</div>
            <p class="soft-note">{escape(t(translations, lang, "kb.rule_diagram_help"))}</p>
            {''.join(html_rows)}
        </div>
        """,
        unsafe_allow_html=True,
...
def render_explanation(result, fact_by_id, recommendations, translations, lang, career_result=None):
    ...
    render_why_not_high(result, translations, lang)
    ...
    render_export_panel(result, fact_by_id, recommendations, translations, lang, "explanation", career_result)
...
def render_kb_viewer(facts, rules, recommendations, references, translations, lang, fact_by_id, career_paths=None):
    ...
    with st.expander(t(translations, lang, "kb.rule_diagram"), expanded=True):
        render_rule_diagram(translations, lang)
...
def render_app_styles():
    st.markdown(
        ...
        .logic-board, .export-panel, .why-card {
            border: 1px solid rgba(148, 163, 184, 0.18);
            border-radius: 8px;
            background: rgba(15, 23, 42, 0.78);
        ...
        .logic-row {
            display: grid;
            grid-template-columns: minmax(180px, 2fr) 52px minmax(76px, 0.6fr) 52px minmax(160px, 1.4fr);
            gap: 8px;
            align-items: center;
            margin: 8px 0;
        }
...
def render_results_view(result, recommendations, translations, lang, career_result=None, fact_by_id=None):
    ...
    if fact_by_id is not None:
        render_export_panel(result, fact_by_id, recommendations, translations, lang, "results", career_result)
...
def main():
    ...
    if current_view == "assessment":
        ...
        render_results_view(result, recommendations, translations, lang, career_result, fact_by_id)
